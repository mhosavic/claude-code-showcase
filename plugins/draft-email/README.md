...
- **Skill name vs invocation**: the **directory name** is what becomes the skill identifier, not the `name` field in frontmatter. So `skills/draft/` plus plugin `draft-email` produces the slash command `/draft-email:draft` (`<plugin-name>:<directory-name>`). The `name` field is the display name only — useful when you want a human-readable label that differs from the directory.
